...
class NfqwsConfigGenerator:
    ...
    def generate_nfqs_args(self, include_ipv6: bool = False, include_quic: bool = True) -> List[str]:
        ...
        # ТОЛЬКО к IP/доменам из --hostlist файлов.
        # Остальной трафик пропускается БЕЗ изменений.
        # Это позволяет работать с приложениями типа Discord, которые используют
        # нестандартные порты (UDP 50000-50100 для голоса, TCP для WebSocket и т.д.)
        args.append("--wf-tcp=*")  # Перехватывать все TCP
        args.append("--wf-udp=*")  # Перехватывать все UDP
