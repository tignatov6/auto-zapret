...
class LogParser:
    ...
    # Zapret (оригинал): example.com : profile 3 : client 192.168.1.1:12345 : proto TLS : fail counter 2/3
    # Zapret2: example.com : profile 3 (name) : client 192.168.1.1:12345 : proto TLS : fail counter 2/3
    ...
    FAIL_COUNTER_PATTERN: Pattern = re.compile(
        ...
        r'(?P<domain>[\w\.\-\*]+)\s*:\s*profile\s+(?P<profile>\d+)\s*(?:\([^)]*\))?\s*:'
        r'\s*client\s+(?P<client>[\w\.\:]+)\s*:\s*proto\s+(?P<proto>\w+)\s*:'
        r'\s*fail counter\s+(?P<counter>\d+)/(?P<threshold>\d+)',
        re.IGNORECASE
    )

    # Пример: example.com : profile 3 (name) : client 192.168.1.1:12345 : proto TLS : adding to /path/to/file.txt
    ...
    # Пример: example.com : profile 3 (name) : client 192.168.1.1:12345 : proto TLS : NOT adding, duplicate detected
    ...
    # Пример: example.com : profile 3 (name) : client 192.168.1.1:12345 : proto TLS : fail counter reset
...
class LogSimulator:
    ...
    def _format_event(self, event: AutoHostlistEvent) -> str:
        """Форматирование события в строку лога (формат Zapret2)"""
        # Zapret2 формат: domain : profile N (name) : client ... : proto ... : message
        profile_str = f"profile {event.profile_id}"
        if event.event_type == EventType.FAIL_COUNTER:
            return (f"{event.domain} : {profile_str} : "
