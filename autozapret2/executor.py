"""
Executor - модуль применения стратегий (Zapret2)
Управление hostlist файлами и отправка SIGHUP nfqws2/winws2
"""
...
# Пути к winws2 и файлам фейков (для Windows, Zapret2)
# BASE_DIR для autozapret2 - корень проекта (где есть bin/, data/, logs/)
# autozapret2/executor.py -> autozapret2/ -> auto-zapret/ (корень)
BASE_DIR = Path(__file__).parent.parent.resolve()
WINWS_PATH = BASE_DIR / "bin" / "zapret2" / "winws2.exe"
WINWS_NAME = "winws2.exe"
FAKE_QUIC_PATH = BASE_DIR / "bin" / "zapret2" / "files" / "fake" / "quic_initial_www_google_com.bin"
FAKE_TLS_PATH = BASE_DIR / "bin" / "zapret2" / "files" / "fake" / "tls_clienthello_www_google_com.bin"

# Время ожидания после запуска winws2 (оптимизировано на основе тестов)
# Формула: max(измеренное) × 1.33 = оптимальное (запас 33%)
# Измерено 2026-03-23: max_start=367ms → (367-50) × 1.33 = 0.42s
# УМЕНЬШЕНО ДО 0.3с как minsleep в blockcheck2.sh для ускорения брутфорса
# Время ожидания инициализации WinDivert + Lua (как minsleep в blockcheck2.sh)
# blockcheck2.sh: minsleep = 100ms, но для первого запуска нужно больше
INIT_WAIT = 0.5  # Для тестирования стратегий (быстро)
INIT_WAIT_FULL = 3.0  # Для первого полного запуска (WinDivert + Lua + autohostlist)
...
class Executor:
    ...
    def __init__(self, config: Optional[Config] = None):
        ...
        # DNS кэш для резолвинга доменов (TTL 1 час)
    ...
    def _write_domain_to_file(self, filepath: str, domain: str) -> None:
        """Атомарная запись домена в файл (для asyncio.to_thread)"""
        # Write to temp file first, then atomic rename
        tmp_file = filepath + ".tmp"
        try:
            # If the file exists, read its content and append
            existing_content = ""
    ...
    def _find_nfqws_pid(self) -> Optional[int]:
        ...
        # Пробуем найти winws2 (Windows)
        try:
            import subprocess
            result = subprocess.run(
                ["wmic", "process", "where", f"name='{WINWS_NAME}'", "get", "processid"],
                capture_output=True,
                text=True,
                timeout=5
            )
    ...
    def _find_fake_files(self) -> Tuple[Optional[Path], Optional[Path], Optional[Path], Optional[Path]]:
        ...
        Returns:
            (fake_quic, fake_tls, fake_http, fake_syndata) - пути к файлам или None
        """
        possible_paths = [
            BASE_DIR / "bin" / "zapret2" / "files" / "fake",
    ...
    def _kill_winws(self) -> None:
        """
        Жёстко убить все процессы winws2.exe
    ...
    def _build_winws_cmd(self, strategy_params: str, domain: Optional[str] = None, is_quic: bool = False) -> List[str]:
        ...
        - Zapret1: --dpi-desync=fake --dpi-desync-ttl=3
        - Zapret2: --lua-desync=fake:blob=fake_default_tls:ip4_ttl=3
        """
        # Проверяем формат параметров
        if "--lua-desync=" in strategy_params:
            # Zapret2 формат — передаём параметры напрямую
            return self._build_winws_cmd_lua(strategy_params, domain, is_quic)
        else:
            # Zapret1 формат — парсим и конвертируем
            strat = self._parse_strategy_params(strategy_params)
            return self.build_cmd(strat, domain, is_quic=is_quic)

    def _build_winws_cmd_lua(self, strategy_params: str, domain: Optional[str] = None, is_quic: bool = False) -> List[str]:
        """
        Построение команды для Zapret2 формата (--lua-desync=...)
        """
        try:
            cmd = [str(WINWS_PATH)]

            # Базовые фильтры
            if is_quic or "udp" in strategy_params.lower() or "quic" in strategy_params.lower():
                cmd.append("--wf-udp=443")
            else:
                cmd.append("--wf-tcp=80,443")

            # Lua init
            lua_dir = self.config.zapret2_lua_dir if hasattr(self.config, 'zapret2_lua_dir') else None
            if lua_dir:
                ...
                if os.path.exists(zapret_lib):
                    cmd.append(f"--lua-init=@{zapret_lib}")
                if os.path.exists(antidpi_lib):
                    cmd.append(f"--lua-init=@{antidpi_lib}")

            # Файлы фейков
            fake_quic, fake_tls, fake_http, fake_syndata = self._find_fake_files()

            # UDP профиль для QUIC
            if is_quic or "udp" in strategy_params.lower() or "quic" in strategy_params.lower():
                cmd.append("--filter-udp=443")
                if fake_quic:
                    cmd.append(f"--lua-desync=fake:blob=fake_default_quic:repeats=4")
                else:
                    cmd.append("--lua-desync=multisplit:pos=1:repeats=2")

                # QUIC ip6_hopbyhop:ip6_destopt
                cmd.append("--new")
                cmd.append("--filter-udp=443")
                cmd.append("--lua-desync=send:ip6_hopbyhop:ip6_destopt")
            else:
                # TCP профиль с фейками
                cmd.append("--filter-udp=443")
                if fake_quic:
                    cmd.append(f"--lua-desync=fake:blob=fake_default_quic:repeats=4")
                if fake_tls:
                    cmd.append(f"--lua-desync=fake:blob=fake_default_tls:repeats=4")
                if fake_http:
                    cmd.append(f"--lua-desync=fake:blob=fake_default_http:repeats=4")

            # TCP профиль с стратегией
            if not is_quic:
                cmd.append("--new")
                cmd.append("--filter-tcp=443")

                # Добавляем параметры стратегии как есть
                cmd.extend(strategy_params.split())

                # IP set для домена
                if domain:
                    ipset_file = BASE_DIR / "data" / "temp_test_ipset.txt"
                    cmd.append(f"--ipset={ipset_file}")

            return cmd
        except FileNotFoundError:
            logger.error(f"winws2.exe not found at {WINWS_PATH}")
            raise
        except PermissionError:
            logger.error(f"Permission denied running winws2.exe")
            raise
        except Exception as e:
            logger.error(f"Failed to build winws command: {e}")
            raise
    ...
    async def stop_winws(self) -> Tuple[bool, str]:
        ...
        async with self._process_lock:
            ...
            # Убиваем все процессы winws2 через taskkill (Windows)
            if sys.platform == 'win32':
                try:
                    subprocess.run(
                        ['taskkill', '/F', '/IM', WINWS_NAME],
    ...
    async def start_winws_with_strategy(
        ...
        """
        logger.info(f"[executor] Starting winws2 with strategy: {strategy_params[:60]}...")

        async with self._process_lock:
            # УБИВАЕМ winws2 ПЕРЕД запуском новой стратегии (как в инструкции)
            self._kill_winws()
            ...
            logger.info(f"[executor] Запуск команды: {' '.join(cmd)}")

            start_time = time.time()

            try:
                # CREATE_NO_WINDOW = 0x08000000 для Windows
                # Но для winws2 нужно DETACHED_PROCESS чтобы он работал в фоне
                ...
                logger.error(f"[executor] winws2.exe not found at {WINWS_PATH}")
                return False, "winws2.exe not found", 0.0
            except PermissionError:
                logger.error(f"[executor] Permission denied running winws2.exe")
                return False, "Permission denied", 0.0
            except Exception as e:
                logger.error(f"[executor] Failed to start winws2: {e}")
                return False, str(e), 0.0

            startup_time = time.time() - start_time
            logger.debug(f"[executor] winws2 startup time: {startup_time*1000:.1f}ms")

            # Ждём INIT_WAIT (0.5с как minsleep в blockcheck2.sh)
            # Для тестирования стратегий — быстро
            ...
            if measure_startup:
                poll = process.poll()
                if poll is not None:
                    # Процесс упал - читаем stderr
                    _, stderr_bytes = process.communicate()
                    stderr_text = stderr_bytes.decode('utf-8', errors='replace').strip() if stderr_bytes else 'no output'
                    logger.warning(f"[executor] winws crashed with code {poll}")
                    logger.warning(f"[executor] stderr: {stderr_text}")
                    self._winws_process = None
                    return False, f"winws2 crashed (code {poll}): {stderr_text[:200]}", startup_time

                # Кэшируем измеренное время если ещё нет
                if self._startup_time is None:
                    self._startup_time = startup_time
                    logger.info(f"[executor] Measured winws2 startup time: {startup_time*1000:.0f}ms")

            logger.info(f"[executor] winws2 started successfully")
            return True, f"winws2 started", startup_time
    
    @profiler
    def get_startup_wait_time(self) -> float:
        """
        Получить время ожидания после запуска winws2
    ...
    async def is_winws_running(self) -> bool:
        """Проверка что winws2 процесс запущен и жив"""
    ...
    async def restart_winws_full(self, nfqws_generator) -> Tuple[bool, str]:
        """Полный рестарт winws2 со ВСЕМИ стратегиями (как в батнике)"""

        # ЗАЩИТА ОТ ОДНОВРЕМЕННОГО РЕСТАРТА СРАЗУ НЕСКОЛЬКИМИ ДОМЕНАМИ
        async with self._full_restart_lock:
            logger.info("[executor] Полный рестарт winws2 со всеми профилями...")
            ...
            if len(nfqws_generator._profiles) == 0:
                ...
                # Базовая стратегия: fake + multisplit + tcp_md5 (универсальная для Zapret2)
                full_args.extend([
                    "--new",
                    "--filter-tcp=443",
            ...
            logger.info(f"[executor] Запуск winws2 с {len(full_args)} аргументами")

            # Лог winws2 stderr — пишем сами через threading
            winws_log_file = BASE_DIR / "logs" / "winws2.log"
            if winws_log_file.exists():
                try:
                    winws_log_file.unlink()
                except Exception:
                    pass
            ...
            try:
                ...
                async with self._process_lock:
                    ...
                    # Запускаем фоновую запись stderr в файл
                    import threading
                    def log_stderr():
                        try:
                            with open(winws_log_file, 'w', encoding='utf-8') as f:
                                for line in process.stderr:
                                    f.write(line.decode('utf-8', errors='replace'))
                                    f.flush()
                        except Exception as e:
                            logger.debug(f"[executor] stderr logger stopped: {e}")
                    threading.Thread(target=log_stderr, daemon=True).start()

                    # Ждём инициализации WinDivert (полный запуск — больше времени)
                    await asyncio.sleep(INIT_WAIT_FULL)

                    # ПРОВЕРЯЕМ что процесс жив (используем локальную переменную!)
                    poll = process.poll()
                    if poll is not None:
                        # Процесс упал! Ждём завершения потока stderr
                        ...
                        if winws_log_file.exists():
                            try:
                                error_text = winws_log_file.read_text(encoding='utf-8', errors='replace').strip()
                            except Exception:
                                pass
                        
                        logger.error(f"[executor] winws2 crashed immediately with code {poll}")
                        if error_text:
                            logger.error(f"[executor] winws2 stderr (last 500 chars): {error_text[-500:]}")
                        self._winws_process = None
                        return False, f"winws2 crashed with code {poll}"

                logger.info(f"[executor] winws2 запущен (pid={process.pid})")
                return True, "winws2 started successfully"
            except FileNotFoundError:
                logger.error(f"[executor] winws2.exe not found at {WINWS_PATH}")
                return False, f"winws2.exe not found at {WINWS_PATH}"
            except Exception as e:
