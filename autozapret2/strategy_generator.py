"""
Strategy Generator - Генерация стратегий для перебора (Zapret2)

Основан на логике blockcheck2.sh из zapret2
Генерирует стратегии для DPI обхода с поддержкой --lua-desync

Включает все режимы из blockcheck2.sh:
- HTTP basic: lua-desync http_hostcase, http_domcase, http_methodeol
- Misc: tcpseg с ip_id=rnd
- OOB: oob:urp=...
- Multi: multisplit/multidisorder
- SeqOvl: tcpseg с seqovl
- Syndata: syndata
- Fake: fake с TTL/autottl/fooling
- Fakedsplit: fakedsplit/fakeddisorder
- Hostfakesplit: hostfakesplit variations
- Fake+Multi: fake + multisplit/multidisorder
- Fake+Faked: fake + fakedsplit/fakeddisorder
- Fake+Hostfake: fake + hostfakesplit
- QUIC: QUIC fake/repeats/ipfrag
"""
...
from .helpers import build_lua_desync_params, get_all_fake_files
...
class StrategyConfig:
    """Конфигурация стратегии (поддержка Zapret1 и Zapret2)"""
    name: str
    ...
    fool: Optional[str] = None  # fooling: ts, md5sig, badsum, badseq, datanoack (Zapret1) или tcp_md5, tcp_ts=-1000 (Zapret2)
    ...
    lua_desync: Optional[str] = None  # Прямая строка lua-desync
    blob: Optional[str] = None  # blob имя

    # Дополнительные параметры
    extra: List[str] = field(default_factory=list)

    @profiler
    def to_params(self, engine: str = "zapret2") -> str:
        """
        Преобразовать в параметры командной строки

        Args:
            engine: "zapret1" или "zapret2"
        """
        if engine == "zapret2":
            return self._to_params_zapret2()
        else:
            return self._to_params_zapret1()

    def _to_params_zapret2(self) -> str:
        """Генерация параметров для Zapret2 (--lua-desync формат)"""
        params = []

        # Если есть прямая lua_desync строка, используем её
        if self.lua_desync:
            params.append(f"--lua-desync={self.lua_desync}")
        else:
            # Строим lua-desync из компонентов
            lua_params = {}

            # Blob
            if self.blob:
                lua_params["blob"] = self.blob
            elif self.mode in ["fake", "fakedsplit", "fakeddisorder"]:
                lua_params["blob"] = "fake_default_tls"
            elif self.mode == "syndata":
                lua_params["blob"] = "fake_default_syndata"

            # Fooling (конвертируем в Zapret2 формат)
            if self.fool:
                fool_map = {
                    "ts": "tcp_ts=-1000",
                    "md5sig": "tcp_md5",
                ...
                lua_params["fooling"] = fool_map.get(self.fool, self.fool)

            # TTL
            if self.ttl:
                lua_params["ip4_ttl"] = str(self.ttl)

            # Position
            if self.pos:
                lua_params["pos"] = self.pos

            # Repeats
            if self.rep and self.rep > 1:
                lua_params["repeats"] = str(self.rep)

            # Wssize
            if self.wssize:
                wsize_parts = self.wssize.split(":")
                lua_params["wsize"] = wsize_parts[0]
                if len(wsize_parts) > 1:
                    lua_params["scale"] = wsize_parts[1]

            # SeqOvl
            if self.seqovl:
                lua_params["seqovl"] = str(self.seqovl)

            # AutoTTL
            if self.autottl:
                lua_params["autottl"] = self.autottl

            # TLS модификаторы
            if self.fake_tls_mod:
                lua_params["tls_mod"] = self.fake_tls_mod

            # Определяем режим
            desync_mode = self.mode
            if self.seqovl:
                desync_mode = "tcpseg"
            elif self.wssize:
                desync_mode = "wssize"

            lua_desync_str = build_lua_desync_params(desync_mode, **lua_params)
            params.append(f"--lua-desync={lua_desync_str}")

        # Payload
        payload = self.payload or "tls_client_hello"
        params.append(f"--payload={payload}")
    ...
    def _to_params_zapret1(self) -> str:
        """Генерация параметров для Zapret1 (обратная совместимость)"""
        params = []
...
class StrategyGenerator:
    ...
    def generate_all(self, include_ipv6: bool = True) -> List[StrategyConfig]:
        ...
        logger.info("Generating strategies (blockcheck2.sh order)...")

        # Phase 10: HTTP basic (http_hostcase, http_domcase, http_methodeol)
        strategies.extend(self._generate_http_basic())

        # Phase 15: Misc (tcpseg:pos=0,method+2:ip_id=rnd:repeats=1,20,100,260)
        strategies.extend(self._generate_misc())

        # Phase 17: OOB (oob:urp=b,0,2,midsld)
        strategies.extend(self._generate_oob())

        # Phase 20: Multi (multisplit/multidisorder)
        strategies.extend(self._generate_multi())

        # Phase 23: SeqOvl
        strategies.extend(self._generate_seqovl())

        # Phase 24: Syndata
        strategies.extend(self._generate_syndata())

        # Phase 25: Fake (TTL, autottl, fooling)
        strategies.extend(self._generate_fake())

        # Phase 30: Faked (fakedsplit/fakeddisorder)
        strategies.extend(self._generate_faked())

        # Phase 35: Hostfake
        strategies.extend(self._generate_hostfake())

        # Phase 50: Fake+Multi
        strategies.extend(self._generate_fake_multi())

        # Phase 55: Fake+Faked
        strategies.extend(self._generate_fake_faked())

        # Phase 60: Fake+Hostfake
        strategies.extend(self._generate_fake_hostfake())

        # Phase 90: QUIC
        strategies.extend(self._generate_quic())

        # Ultimate стратегии из файла (в конце как дополнительные)
    ...
    @profiler
    def _generate_http_basic(self) -> List[StrategyConfig]:
        """Phase 10: HTTP basic (http_hostcase, http_domcase, http_methodeol)"""
        strategies = []
        for mode in ['http_hostcase', 'http_hostcase:spell=hoSt', 'http_domcase', 'http_methodeol', 'http_unixeol']:
            strategies.append(StrategyConfig(
                name=f"http_basic_{mode.replace(':', '_')}",
                mode=mode.split(':')[0],
                extra=[f"--lua-desync={mode}", "--payload=http_req"]
            ))
        return strategies

    def _generate_misc(self) -> List[StrategyConfig]:
        """Phase 15: Misc (tcpseg:pos=0,method+2:ip_id=rnd:repeats=1,20,100,260)"""
        strategies = []
        for repeats in [1, 20, 100, 260]:
            for pos in ['0,method+2', '0,midsld']:
                strategies.append(StrategyConfig(
                    name=f"misc_tcpseg_R{repeats}_{pos.replace(',', '_')}",
                    mode="tcpseg",
                    extra=[f"--lua-desync=tcpseg:pos={pos}:ip_id=rnd:repeats={repeats}"]
                ))
        return strategies

    def _generate_oob(self) -> List[StrategyConfig]:
        """Phase 17: OOB (oob:urp=b,0,2,midsld)"""
        strategies = []
        for urp in ['b', '0', '2', 'midsld']:
            strategies.append(StrategyConfig(
                name=f"oob_urp{urp}",
                mode="oob",
                extra=[f"--lua-desync=oob:urp={urp}", "--in-range=-s1"]
            ))
        return strategies

    def _generate_multi(self) -> List[StrategyConfig]:
        """Phase 20: Multi (multisplit/multidisorder)"""
        strategies = []
        splits_tls = ['2', '1', 'sniext+1', 'sniext+4', 'host+1', 'midsld', '1,midsld', '1,midsld,1220', '1,sniext+1,host+1,midsld-2,midsld,midsld+2,endhost-1']
        for mode in ['multisplit', 'multidisorder']:
            for pos in splits_tls:
                strategies.append(StrategyConfig(
                    ...
                    mode=mode,
                    pos=pos,
                    extra=[f"--lua-desync={mode}:pos={pos}", "--payload=tls_client_hello"]
                ))
        return strategies

    def _generate_seqovl(self) -> List[StrategyConfig]:
        """Phase 23: SeqOvl"""
        strategies = []
        # tcpseg:pos=0,-1:seqovl=1 + drop
        strategies.append(StrategyConfig(
            name="seqovl_tcpseg_1",
            mode="tcpseg",
            seqovl=1,
            extra=["--lua-desync=tcpseg:pos=0,-1:seqovl=1", "--lua-desync=drop"]
        ))
        # multisplit с seqovl
        for pos in ['10', '10,sniext+1', '10,sniext+4', '10,midsld']:
            strategies.append(StrategyConfig(
                name=f"seqovl_multi_{pos.replace(',', '_')}",
                mode="multisplit",
                pos=pos,
                seqovl=1,
                extra=[f"--lua-desync=multisplit:pos={pos}:seqovl=1", "--payload=tls_client_hello"]
            ))
        return strategies

    def _generate_syndata(self) -> List[StrategyConfig]:
        """Phase 24: Syndata"""
        strategies = []
        for mode in ['syndata', 'syndata:blob=0x1603', 'syndata:blob=fake_default_tls:tls_mod=rnd,dupsid,rndsni', 'syndata:blob=fake_default_tls:tls_mod=rnd,dupsid,sni=google.com']:
            strategies.append(StrategyConfig(
                name=f"syndata_{mode.replace(':', '_').replace(',', '_')}",
                mode="syndata",
                extra=[f"--lua-desync={mode}"]
            ))
            # syndata + multisplit
            strategies.append(StrategyConfig(
                name=f"syndata_{mode.replace(':', '_').replace(',', '_')}_multi",
                mode="syndata",
                extra=[f"--lua-desync={mode}", "--lua-desync=multisplit", "--payload=tls_client_hello"]
            ))
        return strategies

    def _generate_fake(self) -> List[StrategyConfig]:
        """Phase 25: Fake (TTL, autottl, fooling) with ALL fake files"""
        strategies = []

        # Get all available TLS fake files
        fake_files = get_all_fake_files()
        tls_fakes = fake_files.get("tls", ["fake_default_tls"])

        # Use first 5 most important TLS fakes to keep strategy count reasonable
        priority_tls_fakes = [f for f in tls_fakes if "iana_org" in f or "google" in f][:5]
        if not priority_tls_fakes:
            priority_tls_fakes = ["fake_default_tls"]

        # Fake с TTL для каждого fake файла
        for fake_file in priority_tls_fakes:
            for ttl in [1, 2, 3, 4, 5, 8]:
                blob_name = fake_file.replace(".bin", "") if fake_file.endswith(".bin") else fake_file
                strategies.append(StrategyConfig(
                    name=f"fake_{blob_name}_ttl{ttl}",
                    mode="fake",
                    ttl=ttl,
                    extra=[f"--lua-desync=fake:blob={blob_name}:ip4_ttl={ttl}:repeats=4", "--payload=tls_client_hello"]
                ))

        # Fake с fooling
        for fooling in ['tcp_md5', 'tcp_ts=-1000']:
            strategies.append(StrategyConfig(
                name=f"fake_{fooling.replace('=', '_').replace('-', 'm')}",
                mode="fake",
                extra=[f"--lua-desync=fake:blob=fake_default_tls:{fooling}:repeats=4", "--payload=tls_client_hello"]
            ))

        # Fake с autottl
        for autottl in [1, 2, 3, 4, 5, 8]:
            strategies.append(StrategyConfig(
                name=f"fake_autottl{autottl}",
                mode="fake",
                extra=[f"--lua-desync=fake:blob=fake_default_tls:ip4_autottl=-{autottl},3-20:repeats=4", "--payload=tls_client_hello"]
            ))

        return strategies

    def _generate_faked(self) -> List[StrategyConfig]:
        """Phase 30: Faked (fakedsplit/fakeddisorder)"""
        strategies = []
        splits = ['2', '1', 'sniext+1', 'sniext+4', 'host+1', 'midsld']
        for mode in ['fakedsplit', 'fakeddisorder']:
            for ttl in [1, 2, 3, 4, 5]:
                for pos in splits:
                    strategies.append(StrategyConfig(
                        name=f"{mode}_ttl{ttl}_{pos.replace('+', '_')}",
                        mode=mode,
                        pos=pos,
                        ttl=ttl,
                        extra=[f"--lua-desync={mode}:pos={pos}:ip4_ttl={ttl}:repeats=4", "--payload=tls_client_hello"]
                    ))
        return strategies

    def _generate_hostfake(self) -> List[StrategyConfig]:
        """Phase 35: Hostfake (hostfakesplit)"""
        strategies = []
        for pos in ['host+1', 'midsld', 'host+1,midsld']:
            strategies.append(StrategyConfig(
                name=f"hostfake_{pos.replace(',', '_')}",
                mode="hostfakesplit",
                pos=pos,
                extra=[f"--lua-desync=hostfakesplit:pos={pos}", "--payload=tls_client_hello"]
            ))
        return strategies

    def _generate_fake_multi(self) -> List[StrategyConfig]:
        """Phase 50: Fake+Multi"""
        strategies = []
        for mode in ['fake,multisplit', 'fake,multidisorder']:
            for pos in ['2', '1', 'sniext+1', 'midsld']:
                strategies.append(StrategyConfig(
                    name=f"{mode.replace(',', '_')}_{pos}",
                    mode=mode,
                    pos=pos,
                    extra=[f"--lua-desync={mode}:pos={pos}:repeats=4", "--payload=tls_client_hello"]
                ))
        return strategies

    def _generate_fake_faked(self) -> List[StrategyConfig]:
        """Phase 55: Fake+Faked"""
        strategies = []
        for mode in ['fake,fakedsplit', 'fake,fakeddisorder']:
            for pos in ['2', '1', 'sniext+1', 'midsld']:
                strategies.append(StrategyConfig(
                    name=f"{mode.replace(',', '_')}_{pos}",
                    mode=mode,
    ...
    def _generate_fake_hostfake(self) -> List[StrategyConfig]:
        """Phase 60: Fake+Hostfake"""
        strategies = []
        for pos in ['host+1', 'midsld']:
            strategies.append(StrategyConfig(
                name=f"fake_hostfake_{pos.replace('+', '_')}",
                mode="fake,hostfakesplit",
                pos=pos,
                extra=[f"--lua-desync=fake,hostfakesplit:pos={pos}:repeats=4", "--payload=tls_client_hello"]
            ))
        return strategies

    def _generate_quic(self) -> List[StrategyConfig]:
        """Phase 90: QUIC"""
        strategies = []
        # QUIC fake с разными repeats
        for rep in [1, 2, 4, 8]:
            strategies.append(StrategyConfig(
                name=f"quic_fake_R{rep}",
                mode="fake",
                rep=rep,
                extra=[f"--lua-desync=fake:blob=fake_default_quic:repeats={rep}"]
            ))
        # QUIC send:ip6_hopbyhop:ip6_destopt
        strategies.append(StrategyConfig(
            name="quic_ip6_hopbyhop",
            mode="send",
            extra=["--lua-desync=send:ip6_hopbyhop:ip6_destopt"]
        ))
        return strategies
