...
for old_log in [log_file, log_dir / "autohostlist.log", log_dir / "winws2.log"]:
    if old_log.exists():
        try:
            old_log.unlink()
        except Exception:
...
class AutoZapretApp:
    ...
    async def initialize(self) -> None:
        ...
        # Проверяем и запускаем winws2 если не запущен
    ...
    async def _ensure_winws_running(self) -> None:
        """Проверка и запуск winws2 если не запущен"""
        ...
        if winws_pid is None:
            logger.info("winws2 is not running. Starting winws2 with default strategy...")
            ...
            if strategies:
                ...
                if success:
                    logger.info(f"winws2 started successfully with strategy '{strategies[0].name}'")
                else:
                    logger.warning(f"Failed to start winws2: {msg}")
            else:
                logger.info("No strategies available, winws2 will be started on-demand when needed")
        else:
    ...
    async def shutdown(self) -> None:
        ...
        # 3. Останавливаем winws2 если он запущен
        if self.executor:
...
def cli(ctx, config_dir: Optional[str]):
    """Auto-Zapret (Zapret2) - Адаптивная мультистратегическая маршрутизация"""
...
def serve(ctx, host, port, log_level):
    ...
    click.echo(f"Starting Auto-Zapret (Zapret2) Web UI at http://{host}:{port}")
...
def status(ctx):
    ...
    click.echo("Auto-Zapret (Zapret2) Status")
    click.echo("=" * 40)
...
def stats(ctx):
    ...
    async def run_async():
        ...
        try:
            ...
            click.echo("\nAuto-Zapret (Zapret2) Statistics")
            click.echo("=" * 40)
