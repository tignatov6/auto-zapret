...
DEFAULT_TEST_TIMEOUT = 10.0  # seconds - unified default timeout
...
# Параллелизм - ОДИН домен за раз (winws2 не поддерживает параллельные стратегии)
...
class CalibrationResult:
    ...
    timeout_base: float = DEFAULT_TEST_TIMEOUT  # Базовый таймаут для HTTPS теста
    timeout_extended: float = DEFAULT_TEST_TIMEOUT  # Расширенный таймаут для сложных тестов
...
class StrategyTester:
    ...
    async def _get_session(self) -> aiohttp.ClientSession:
        ...
        if self._session is None or self._session.closed:
            timeout = aiohttp.ClientTimeout(
                total=DEFAULT_TEST_TIMEOUT,
                connect=DEFAULT_TEST_TIMEOUT / 2,
                sock_connect=DEFAULT_TEST_TIMEOUT / 5,
                sock_read=DEFAULT_TEST_TIMEOUT / 2
            )
    ...
    async def calibrate(self, force: bool = False) -> CalibrationResult:
        ...
        if all_rtt:
            ...
            self._calibration = CalibrationResult(
                ...
                fast_sites_rtt=50,
                slow_sites_rtt=150,
                timeout_base=DEFAULT_TEST_TIMEOUT,
                timeout_extended=DEFAULT_TEST_TIMEOUT,
                success_rate=0
            )
            self._calibration_time = time.time()
    ...
    async def _test_strategy_impl(
        ...
        """Реализация тестирования стратегии с ПЕРЕЗАПУСКОМ winws2"""
        start_time = time.time()
        ...
        # ═══════════════════════════════════════════════════════
        # ШАГ 1: Останавливаем winws2 и запускаем с НОВОЙ стратегией
        # ═══════════════════════════════════════════════════════

        # Останавливаем текущий winws2
        logger.debug(f"[test_strategy] Stopping winws2...")
        stop_start = time.time()
        await self.executor.stop_winws()
        timings['stop_winws'] = time.time() - stop_start
        logger.debug(f"[test_strategy] winws2 stopped ({timings['stop_winws']:.3f}s)")

        # Запускаем winws2 с параметрами тестируемой стратегии
        logger.debug(f"[test_strategy] Starting winws2 with strategy...")
        ...
        # ЛОГИРОВАНИЕ ДЛЯ ОТЛАДКИ: что мы передаём в executor
        logger.info(f"[test_strategy] Final params: domain_is_quic={domain_is_quic}, is_quic_strategy={is_quic_strategy}, use_quic_test={use_quic_test}")
        
        success, msg, startup_time = await self.executor.start_winws_with_strategy(
            strategy_params,
            domain=domain,
            measure_startup=is_first_run,
            is_quic=is_quic_strategy  # Для TCP стратегий это False
        )
        timings['start_winws'] = time.time() - start_winws_begin
        
        if success:
            logger.debug(f"[test_strategy] winws2 started ({timings['start_winws']:.3f}s), startup_time={startup_time:.3f}s")
        else:
            logger.warning(f"[test_strategy] Failed to start winws2: {msg}")
            return StrategyTestResult(
                status=StrategyTestStatus.ERROR,
                domain=domain,
                strategy_params=strategy_params,
                error=f"winws2 start error: {msg}"
            )
    ...
    async def _test_strategy_socket(
        ...
        def test_https_curl() -> tuple:
            ...
            try:
                ...
                # Получаем IP-адреса для curl --resolve (ВАЖНО: curl должен подключаться к тому же IP что winws2 обрабатывает!)
                ips = self.executor._resolve_domain_ips(domain)
                if not ips:
                    return False, (time.time() - test_start) * 1000, 'dns_failed'

                # Формируем команду curl с --resolve чтобы curl подключался к IP из ipset
                # Это КРИТИЧЕСКИ ВАЖНО — иначе curl может подключиться к другому IP мимо winws2!
                cmd = [
                    "curl",
                    "-ISs",                    # I=HEAD, S=show errors, s=silent
                    "-4",                      # Только IPv4 (winws2 обрабатывает IPv4)
                    "-A", "curl/7.88.1",       # User-Agent как в blockcheck
                    "--max-time", str(timeout), # Таймаут на ВЕСЬ цикл
                    "--tlsv1.2",               # TLS 1.2
                    "--http1.1",               # Force HTTP/1.1
                    "--resolve", f"{domain}:443:{ips[0]}",  # ВАЖНО: подключаемся к IP из ipset!
