...
class DPIDetector:
    ...
    # Пути к blockcheck.sh / blockcheck2.sh (ищем в нескольких местах)
    BLOCKCHECK_PATHS = [
        "./zaprets-sources/zapret2/blockcheck2.sh",
        "./zapret2/blockcheck2.sh",
        "./zapret-src/zapret-v72.12/blockcheck2.sh",
        "./zapret-src/blockcheck2.sh",
    ...
    def _find_blockcheck(self) -> Optional[Path]:
        """Поиск blockcheck.sh / blockcheck2.sh"""
        # Сначала проверяем явный путь (из конфига)
        if self.explicit_blockcheck_path:
            path = Path(self.explicit_blockcheck_path)
            if path.exists():
                logger.info(f"Using explicit blockcheck at {path}")
                return path

        # Проверяем заданный путь к исходникам (blockcheck2.sh приоритетнее)
        if self.zapret_src_dir:
            path = Path(self.zapret_src_dir) / "blockcheck2.sh"
            if path.exists():
                logger.info(f"Found blockcheck2.sh at {path}")
                return path
        ...
        for path_str in self.BLOCKCHECK_PATHS:
            path = Path(path_str)
            if path.exists():
                logger.info(f"Found blockcheck at {path}")
                return path

        logger.warning("blockcheck.sh / blockcheck2.sh not found")
        return None
    ...
    async def check_domain(self, domain: str, timeout: int = 60) -> DPICheckResult:
        ...
        # Проверяем наличие blockcheck и что мы не на Windows
        if self.blockcheck_path and os.name != 'nt':
            logger.debug(f"[dpi_detector] Using blockcheck at {self.blockcheck_path}")
            try:
                # Запускаем blockcheck
                logger.info(f"[dpi_detector] Running blockcheck for {domain}...")
                ...
                logger.warning(f"[dpi_detector] blockcheck timeout for {domain}")
                return DPICheckResult(
                    domain=domain,
                    status=DPIStatus.ERROR,
                    error="blockcheck timeout",
                    returncode=-1
                )
            except Exception as e:
                logger.warning(f"[dpi_detector] blockcheck failed for {domain}: {e}, using fallback")

        # Fallback: используем встроенний детектор
        logger.warning("[dpi_detector] blockcheck not available, using fallback detector")
        return await self._fallback_check(domain)
    
    @profiler
    async def _run_blockcheck(self, domain: str, timeout: int) -> Tuple[str, int]:
        """
        Запуск blockcheck / blockcheck2.sh
    ...
    def _parse_blockcheck_output(self, domain: str, output: str, returncode: int = 0) -> DPICheckResult:
        """Парсинг вывода blockcheck / blockcheck2.sh"""
        ...
        if returncode == 127:
            return DPICheckResult(
                domain=domain,
                status=DPIStatus.ERROR,
                error="blockcheck not found or not executable",
            ...
            return DPICheckResult(
                domain=domain,
                status=DPIStatus.ERROR,
                error="blockcheck not executable",
                returncode=returncode,
                raw_output=output[:10000]
            )
        elif returncode == 2:
            ...
            return DPICheckResult(
                domain=domain,
                status=DPIStatus.ERROR,
                error=f"blockcheck crashed with code {returncode}",
                returncode=returncode,
                raw_output=output[:10000]
            )
    ...
    async def _fallback_check(self, domain: str) -> DPICheckResult:
        """
        Резервный детектор (если blockcheck не найден)
