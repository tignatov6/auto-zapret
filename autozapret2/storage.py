...
class Strategy:
    ...
    lua_desync: Optional[str] = None  # Direct lua-desync string
    blob: Optional[str] = None  # blob name
...
@dataclass
class StrategyCreateResult:
    """Результат создания стратегии

    Attributes:
        strategy_id: ID стратегии (новой или существующей)
        was_created: True = создана новая стратегия, False = уже существовала
    """
    strategy_id: int
    was_created: bool  # True = создана новая, False = уже существовала
...
class Storage:
            ...
            if existing:
                ...
                cursor = await self._db.execute(
                    """INSERT INTO strategies
                       (name, description, zapret_params, params_canonical, priority, payload, lua_desync, blob, created_at,
                        domains_count, success_rate, total_checks, last_success)
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                    (strategy.name, strategy.description,
                     strategy.zapret_params, params_canonical, strategy.priority,
                     strategy.payload, strategy.lua_desync, strategy.blob,
    ...
    @profiler
    async def create_strategy(self, name: str, params: str, description: str = "") -> StrategyCreateResult:
        """
        ...
        Returns:
            StrategyCreateResult со стратегией ID и флагом was_created
        """
        strategy = Strategy(
            name=name,
        ...
        async with self._lock:
            ...
            if existing_by_name:
                ...
                return StrategyCreateResult(strategy_id=existing_by_name["id"], was_created=False)
            ...
            if params_canonical:
                ...
                if existing_by_params:
                    ...
                    return StrategyCreateResult(strategy_id=existing_by_params["id"], was_created=False)
            ...
            cursor = await self._db.execute(
                """INSERT INTO strategies
                   (name, description, zapret_params, params_canonical, priority, payload, lua_desync, blob, created_at,
                    domains_count, success_rate, total_checks, last_success)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (name, description, params, params_canonical, 99, None, None, None,
                 datetime.now().isoformat(), 0, 0.0, 0, None)
            )
            await self._db.commit()
            return StrategyCreateResult(strategy_id=cursor.lastrowid, was_created=True)
    ...
    async def get_strategy(self, name: str) -> Optional[Strategy]:
        ...
        async with self._lock:
            cursor = await self._db.execute(
                "SELECT id, name, description, zapret_params, params_canonical, priority, payload, lua_desync, blob, created_at, domains_count, success_rate, total_checks, last_success FROM strategies WHERE name = ?", (name,)
            )
    ...
    async def get_strategy_by_id(self, id: int) -> Optional[Strategy]:
        ...
        async with self._lock:
            cursor = await self._db.execute(
                "SELECT id, name, description, zapret_params, params_canonical, priority, payload, lua_desync, blob, created_at, domains_count, success_rate, total_checks, last_success FROM strategies WHERE id = ?", (id,)
            )
            row = await cursor.fetchone()
            return Strategy(**dict(row)) if row else None
    ...
    async def get_strategy_by_params(self, params: str) -> Optional[Strategy]:
        ...
        async with self._lock:
            ...
            cursor = await self._db.execute(
                "SELECT id, name, description, zapret_params, params_canonical, priority, payload, lua_desync, blob, created_at, domains_count, success_rate, total_checks, last_success FROM strategies WHERE params_canonical = ?",
                (params_canonical,)
            )
            row = await cursor.fetchone()
            return Strategy(**dict(row)) if row else None
    ...
    async def list_strategies(self) -> List[Strategy]:
        ...
        async with self._lock:
            cursor = await self._db.execute(
                "SELECT id, name, description, zapret_params, params_canonical, priority, payload, lua_desync, blob, created_at, domains_count, success_rate, total_checks, last_success FROM strategies ORDER BY priority ASC"
            )
    ...
    async def get_domain(self, domain: str) -> Optional[Domain]:
        ...
        async with self._lock:
            domain = normalize_domain(domain)
            cursor = await self._db.execute(
                "SELECT domain, strategy_id, added_at, fail_count, last_fail, is_active FROM domains WHERE domain = ?", (domain,)
            )
            row = await cursor.fetchone()
    ...
    async def get_domain_strategy(self, domain: str) -> Optional[Strategy]:
        ...
        async with self._lock:
            domain = normalize_domain(domain)
            cursor = await self._db.execute(
                """SELECT s.id, s.name, s.description, s.zapret_params, s.params_canonical, s.priority, s.payload, s.lua_desync, s.blob, s.created_at, s.domains_count, s.success_rate, s.total_checks, s.last_success FROM domains d
    ...
    async def list_domains(self, active_only: bool = True) -> List[Domain]:
        ...
        async with self._lock:
            if active_only:
                cursor = await self._db.execute(
                    "SELECT domain, strategy_id, added_at, fail_count, last_fail, is_active FROM domains WHERE is_active = 1 ORDER BY domain"
                )
            else:
                cursor = await self._db.execute(
                    "SELECT domain, strategy_id, added_at, fail_count, last_fail, is_active FROM domains ORDER BY domain"
                )
            rows = await cursor.fetchall()
    ...
    async def get_stats(self, domain: Optional[str] = None,
        ...
        async with self._lock:
            if domain:
                domain = normalize_domain(domain)
                cursor = await self._db.execute(
                    """SELECT id, domain, timestamp, event_type, strategy_id, details FROM stats WHERE domain = ?
                       ORDER BY timestamp DESC LIMIT ?""",
                    (domain, limit)
                )
            else:
                cursor = await self._db.execute(
                    "SELECT id, domain, timestamp, event_type, strategy_id, details FROM stats ORDER BY timestamp DESC LIMIT ?", (limit,)
                )
            rows = await cursor.fetchall()
    ...
    async def get_strategies_by_priority(self, min_success_rate: float = 0.0) -> List[Strategy]:
        ...
        async with self._lock:
            cursor = await self._db.execute(
                """SELECT id, name, description, zapret_params, params_canonical, priority, payload, lua_desync, blob, created_at, domains_count, success_rate, total_checks, last_success FROM strategies
                    ...
                    try:
                        await self._db.execute(
                            ...
                            (strat_data["name"], strat_data.get("description", ""),
                             ...
                             strat_data.get("priority", 99),
                             strat_data.get("payload"), strat_data.get("lua_desync"), strat_data.get("blob"),
                             datetime.now().isoformat(),
                             0, 0.0, 0, None)
                        )
