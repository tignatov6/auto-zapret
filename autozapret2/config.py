...
class Config:
    ...
    # Zapret2 специфичные настройки
    zapret2_lua_dir: str = ""  # Путь к Lua скриптам Zapret2
    zapret2_blobs_dir: str = ""  # Путь к blob файлам Zapret2
    zapret2_engine: str = "zapret2"  # "zapret1" или "zapret2"
    ...
    brute_force_type: str = "exact"  # "aggregated" (~398) или "exact" (~19356)
    ...
    def __post_init__(self):
        """Инициализация путей по умолчанию и валидация параметров"""
        # Валидация brute_force_type
        if self.brute_force_type not in ("aggregated", "exact"):
            raise ValueError(
                f"Invalid brute_force_type: {self.brute_force_type}. Must be 'aggregated' or 'exact'"
            )
        ...
        # Zapret2 пути
        if not self.zapret2_lua_dir:
            self.zapret2_lua_dir = str(self._base_dir / "bin" / "zapret2" / "lua")
        if not self.zapret2_blobs_dir:
            self.zapret2_blobs_dir = str(self._base_dir / "bin" / "zapret2" / "files" / "fake")
