...
from .blockcheck_selector_exact import ExactStrategySelector, ExactSelectorState
...
async def check_dns_spoofing(domain: str) -> dict:
    """
    Check for DNS spoofing by comparing system resolver vs public DNS

    Returns dict with:
    - system_ip: IP from system resolver
    - google_ip: IP from Google DNS (8.8.8.8)
    - cloudflare_ip: IP from Cloudflare DNS (1.1.1.1)
    - is_spoofed: True if IPs differ significantly
    """
    async def resolve_with_dns(domain: str, dns_server: str) -> str:
    ...
    # Public DNS
    google_ip = await resolve_with_dns(domain, "8.8.8.8")
    cloudflare_ip = await resolve_with_dns(domain, "1.1.1.1")

    is_spoofed = (system_ip and google_ip and system_ip != google_ip)
...
def detect_tls_version(domain: str) -> str:
    """
    Detect TLS version for domain
    Returns: 'tls12', 'tls13', or 'unknown'
    """
    try:
        import ssl
        import socket
...
class Analyzer:
    ...
    async def _apply_strategy_to_domain_impl(self, domain: str, protocol: str = "unknown") -> None:
        ...
        if bf_result.status == BruteForceStatus.FOUND and bf_result.params:
            ...
            if existing_strategy:
                ...
                result = await self.storage.create_strategy(
                    name=strategy_name,
    ...
    async def _brute_force_strategies_impl(self, domain: str) -> BruteForceResult:
        ...
        # Выбираем selector: exact (все комбинации blockcheck2) или aggregated
        use_exact = getattr(self.config, 'brute_force_type', 'aggregated') == 'exact'
        
        if use_exact:
            logger.info(f"[brute_force_impl] Using EXACT blockcheck2 selector (all combinations)")
            selector = ExactStrategySelector(protocol=protocol.value, mode=brute_force_mode)
            state = ExactSelectorState()
        else:
            logger.info(f"[brute_force_impl] Using aggregated selector (~398 strategies)")
        ...
        for strategy in selector.generate(state):
            ...
            # Обновляем состояние для адаптивного skip (только для aggregated selector)
            if not use_exact:
