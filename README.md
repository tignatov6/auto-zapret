...
**Auto-Zapret** основан на проекте **[Zapret](https://github.com/bol-van/zapret)** от **bol-van**.

> **Zapret** — автономное средство противодействия DPI, не требующее подключения сторонних серверов.  
> Предназначено для обхода блокировок или замедления сайтов HTTP(S), сигнатурного анализа TCP и UDP протоколов.  
> Лицензия: Open Source (GPL)  
> Автор: **bol-van** | GitHub: [github.com/bol-van/zapret](https://github.com/bol-van/zapret)

Auto-Zapret использует **WinWS** (Windows-версия nfqws) из проекта Zapret для перехвата и модификации трафика.
...
#### Вариант 1: Через BAT-скрипт (Windows, рекомендуется)
...
**Что делает скрипт:**
- ✅ Проверяет права администратора (автоматический UAC запрос)
- ✅ Проверяет установку WinDivert
- ✅ Подготавливает файлы и директории
- ✅ Запускает WinWS с поддержкой autohostlist
- ✅ Запускает Web UI на порту 8000
- ✅ Показывает статус после запуска

**Остановка:** Закройте командное окно скрипта
